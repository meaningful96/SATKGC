...
class CustomBertModel(nn.Module, ABC):
    def __init__(self, args):
        ...
        self.log_inv_a = torch.nn.Parameter(torch.tensor(1.0 / args.a).log(), requires_grad=args.finetune_a)
    ...
    def compute_logits(self, output_dict: dict, batch_dict: dict) -> dict:
        ...
        if args.DegW == False:
            st_vector2 = torch.tensor(dt)
        else:
            st_vector2 = torch.tensor(dh)
        st_weight2 = L2_norm(st_vector2.repeat(len(dh), 1).float())
        st_weight2.fill_diagonal_(1)
        st_weight2 *= self.log_inv_a.exp()
        logits += st_weight2
        ...
        return {'logits': logits,
                ...
                'inv_a': self.log_inv_a.detach().exp(),
